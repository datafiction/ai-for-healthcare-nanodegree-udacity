...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    ndc_dict = dict(zip(ndc_df['NDC_Code'], ndc_df['Non-proprietary Name']))
    df['generic_drug_name'] = df['ndc_code'].map(ndc_dict)
    return df
...
def select_first_encounter(df):
    ...
    '''
    first_encounter_df = df.drop_duplicates('patient_nbr')
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    train_size = round(total_values * (train_percentage))
    vt_size = round(total_values * (val_percentage)) + train_size
